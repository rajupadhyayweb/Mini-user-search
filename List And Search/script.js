let userContainer = document.querySelector('.userContainer'); // User cards rakhne wala container
...
function renderUsers(arr) {
    ...
    userContainer.innerHTML = ''; // Purane users hatao

    if (arr.length === 0) { // Agar koi user nahi mila
    ...
    arr.map(function (obj) {
        ...
        let { profileUrl, name, email } = obj; // Object se values nikalo

        let divElem = document.createElement('div'); // Naya div banao
        ...
        userContainer.append(divElem); // User card page par dikhao
    });
}

renderUsers(users); // Sab users ko display karo

function handleSearch(e) {

    let searchValue = e.target.value; // Input ki value lo

    let filteredUsers = users.filter(obj => { // Matching users nikalo
        return (
    ...
    renderUsers(filteredUsers); // Sirf matching users dikhao
}

searchInput.addEventListener('input', handleSearch); // Typing par search chalao